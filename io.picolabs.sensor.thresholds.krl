ruleset io.picolabs.sensor.thresholds {
  ...
  rule check_violation {
    ...
    pre {
      ...
      msg = under => <<#{name} is under threshold of #{lower_threshold} at #{reading}°F>>
          | over  => <<#{name} is over threshold of #{upper_threshold} at #{reading}°F>>
          |          <<#{name} is between #{lower_threshold} and #{upper_threshold} at #{reading}°F>>;
